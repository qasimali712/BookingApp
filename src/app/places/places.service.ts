...
export class PlacesService {
  ...
  fetchPlaces(): Observable<Place[]> {
    return this.http
      .get<PlaceData>(
        'https://newone-de6b9-default-rtdb.asia-southeast1.firebasedatabase.app/f-places.json'
      )
      .pipe(
        map((res) => {
          const places: Place[] = [];
  ...
  deletePlace(placeId: string): Observable<void> {
    ...
    if (index >= 0) {
      ...
      return this.http
        .delete<void>(
          `https://newone-de6b9-default-rtdb.asia-southeast1.firebasedatabase.app/f-places/${placeId}.json`
        )
        .pipe(
          tap(() => {
    ...
    this.http
      ...
      .subscribe(
        () => {
          ...
          const oldPlaceIndex = updatedPlaces.findIndex(
            (p) => p.id === place.id
          );
